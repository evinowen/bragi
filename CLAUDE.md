...
12. **Use `</dev/tty` for read commands** - When script may be executed via pipe (`curl | bash`), redirect stdin from terminal for user input
...
After running `deploy/deploy.ts` (via `cd deploy && npm run deploy`), always extract and print the following from the output for the user so they can immediately access and log in to the services:
