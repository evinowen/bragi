...
function sshRun(args: string[], opts: SpawnSyncOptions = {}): { status: number; stdout: string; stderr: string } {
  ...
  const result: SpawnSyncReturns<string> = spawnSync(resolvedCmd, resolvedArgs, { encoding: 'utf8', ...opts });
  return {
    status: result.status ?? 1,
...
function loadConfig(): void {
  ...
  const cfg: Config = JSON.parse(fs.readFileSync(CONFIG_FILE, 'utf8'));
